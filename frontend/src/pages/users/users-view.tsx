    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox>
          ...
          <>
            <p className={'block font-bold mb-2'}>Food_entries User</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.food_entries_user &&
                      Array.isArray(users.food_entries_user) &&
                      users.food_entries_user.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(
                              `/food_entries/food_entries-view/?id=${item.id}`,
                            )
                          }
                        >
                          <td data-label='calorie_count'>
                            {item.calorie_count}
                          </td>

                          <td data-label='fat_content'>{item.fat_content}</td>

                          <td data-label='protein_content'>
                            {item.protein_content}
                          </td>

                          <td data-label='carbohydrate_content'>
              ...
              {!users?.food_entries_user?.length && (
                <div className={'text-center py-4'}>No data</div>
              )}
            </CardBox>
          </>
