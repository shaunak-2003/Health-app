...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/food_entries/food_entriesSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureFood_entriesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 10;

const TableSampleFood_entries = ({ filterItems, setFilterItems, filters }) => {
  ...
  const {
    ...
    loading,
    count,
    notify: food_entriesNotify,
    refetch,
  } = useAppSelector((state) => state.food_entries);
  ...
  useEffect(() => {
    if (food_entriesNotify.showNotification) {
      notify(
        food_entriesNotify.typeNotification,
        food_entriesNotify.textNotification,
      );
    }
  }, [food_entriesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/food_entries/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/food_entries/food_entries-view/?id=${id}`);
  };
...
export default TableSampleFood_entries;
